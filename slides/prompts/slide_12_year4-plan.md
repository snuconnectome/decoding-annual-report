...
16:9 horizontal Korean forward-plan infographic, background.

**Top-left header**: "디코딩과제 3년차".

**Title**: "4차년도 (2026) 연구개발계획" .
**Subtitle**: "예산 6억원 (600,000천원) · 5개 핵심 방향".

**상단 주요 영역 (세로 약 55%) — 5개 번호 카드** (동일 너비, 배경, 얇은 테두리):

**카드 01 — 데이터베이스 공개 (BrainLife 플랫폼)**:
- 아이콘: 평면 데이터베이스 실린더
- 제목: "데이터베이스 공개 (BrainLife)"
- 설명: "자연환경 지각·정서 데이터베이스를 BrainLife 오픈 플랫폼으로 공개 · 전 세계 연구자 공유"

**카드 02 — 다중 입력 분포 기계학습 모델**:
- 아이콘: 평면 네트워크 그래프
- 제목: "다중 입력 분포 기계학습"
- 설명: "동일 종에서 다중 모달리티 (구조적·기능적 자기공명영상·뇌전도) 통합 디코더 검증"

**카드 03 — 신경과학 메커니즘 규명 (3년차 누락 항목 재개)**:
- 아이콘: 평면 광선 (two-site 광유전학)
- 제목: "신경과학 메커니즘"
- 설명: "2개 부위 (two-site) 광유전학 + 칼슘이미징 · 감각-정서의 직접적 인과관계 규명"

**카드 04 — 파운데이션 모델 전이학습 검증**:
- 아이콘: 평면 층상 박스
- 제목: "파운데이션 모델 전이학습"
- 설명: "대규모 사전학습 (Pre-training) + 전이학습 (Transfer Learning) 성능 검증"

**카드 05 — 동물-사람 종간 통합**:
- 아이콘: 평면 연결된 종 심볼
- 제목: "동물-사람 종간 통합"
- 설명: "동물 행동·신경 신호 + 사람 기능적 자기공명영상·뇌전도 → 종간 파운데이션 모델 사전학습"

**하단 영역 (세로 약 25%) — 예산 수평 막대** (단일 막대, 높이 약):

- 왼쪽 78% 구간: SNU Blue 채우기 + 흰색 라벨 "직접비 468,750천원 (약 4억 6,875만원)" - 오른쪽 22% 구간: Neural Teal 채우기 + 흰색 라벨 "간접비 131,250천원 (약 1억 3,125만원)" - 막대 위 오른쪽: "합계 600,000천원 (약 6억원)" **막대 하단 캡션**:
