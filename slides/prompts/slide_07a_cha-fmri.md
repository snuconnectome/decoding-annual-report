...
**Title (32pt **: "기능적 자기공명영상 (fMRI) 파운데이션 모델 ①"
**Subtitle (18pt)**: "평가항목 제2·제4번 · 달성도 100% · UKB+HCP+ABCD 약 5만명 사전학습으로 범용 뇌 표상 확보"

**Left accent bar**: ·.

**Top-right Hero 2단**:
- 상단 "대규모 뇌영상 사전학습" - 하단 "UKB + HCP + ABCD 약 5만명 · 88억 파라미터" **Main 2×2 grid** ( 배경, 얇은 테두리, 각 카드 아이콘·도식 좌상단):

**카드 1 — SwiFT**:
- 아이콘: 평면 뇌 + 오름차순 계단
- 헤드라인: "스케일 확대가 다운스트림 성능 개선으로 이어짐을 관찰함"
...
**카드 2 — SwiFT-IO**:
- 아이콘: 영화 프레임 + 뇌 파형
- 헤드라인: "영화 시청 중 연속 정서가를 개인 단위 회귀하는 모델을 확립함"
- 핵심 수치 "R² = 0.96" - **중앙 도식**: 수평 축 (−0.2 ~ 1.0) 위에 두 점 — LSTM 기준선 R² ≈ −0.14 (좌측) · SwiFT-IO R² = 0.96 (우측 끝) · 0.0 기준선 점선
- 2 bullets:
...
**카드 3 — MBBN**:
- 아이콘: 3층 주파수 파형
- 헤드라인: "ADHD 연결성 장애가 주파수 대역별로 분리됨을 관찰함"
